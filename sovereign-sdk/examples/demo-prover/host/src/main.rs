...
use demo_stf::app::{DefaultPrivateKey, NativeAppRunner, DefaultContext};
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    for height in rollup_config.start_height.. {
        ...
        let header_hash = hex::encode(filtered_block.header.hash.clone());
        ...
        let inputs = bincode::serialize(&host.inputs()).expect("Serialization should not fail");
        std::fs::write(format!("inputs_{}.bin", height), inputs).expect("Write should not fail");
