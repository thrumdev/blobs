...
use pallet_ikura_length_fee_adjustment::NextLengthMultiplier;
...
fn create_submit_blob_utx(
    ...
    let signed_extra: SignedExtra = (
        ...
        pallet_ikura_blobs::PrevalidateBlobs::<Runtime>::new(),
    );

    let runtime_call: RuntimeCall = pallet_ikura_blobs::Call::submit_blob {
        ...
        assert_eq!(
            ...
            Err(TransactionValidityError::Invalid(
                InvalidTransaction::Custom(
                    ikura_primitives::InvalidTransactionCustomError::BlobExceedsSizeLimit as u8,
                )
            ))
        )
    });
...
fn test_pre_dispatch_max_blobs_exceeded() {
    test_pre_dispatch(|| pallet_ikura_blobs::TotalBlobs::<Runtime>::put(MaxBlobs::get()));
}

#[test]
fn test_pre_dispatch_max_total_blob_size_exceeded() {
    test_pre_dispatch(
        || pallet_ikura_blobs::TotalBlobSize::<Runtime>::put(MaxTotalBlobSize::get()),
    );
}
...
fn test_inclusion_fee_uses_length_to_fee() {
    ...
    new_test_ext().execute_with(|| {
        let call: RuntimeCall = pallet_ikura_blobs::Call::submit_blob {
            namespace_id: 0.into(),
            blob: vec![0; 1],
        }
        .into();
