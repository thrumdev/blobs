...
enum Commands {
    ...
    /// Generate a list of transfers and output a blob.
    Avalanche {
        number: u32,
    },
    /// Utility commands
...
mod avalanche {
    ...
    impl Account {
        fn generate() -> Self {
            Self {
                priv_key: DefaultPrivateKey::generate(),
                amount: 0,
                nonce: 0,
            }
        }

        /// Returns the root of the avalanche. This is the address of the sequencer and has
        /// a balance of 1000.
        fn root() -> Self {
            let addr = DefaultPrivateKey::from_hex("ca4abe0571a6e8e8f2caf8a3229a5cb210260ba95e4f0ab2b1348c8a67c93bf1e000b7f860471831d0c7fa61fd348e2347dad47d72131bf8a2c8234bb7b53a9c").unwrap();
            Self {
                priv_key: addr,
                amount: 1000,
                nonce: 0,
    ...
    /// Creates a list of random transfers and outputs a list of hex encoded transactions.
    pub(super) fn gen(number: u32) -> Vec<String> {
        use rand::Rng;

        let mut result = vec![];
        let mut rng = rand::thread_rng();
        let mut accounts = vec![Account::root()];
        for _ in 0..number {
            // Pick a random source.
            let source_idx = rng.gen_range(0..accounts.len());

            // Pick a random destination.
            let dest_idx = {
                let create_new = rng.gen_bool(0.5);
                if create_new {
            ...
            let amount = rng.gen_range(1..=accounts[source_idx].amount);
            ...
            // }
            let token_address =
                    sov_bank::create_token_address::<C>(
                        "sov-demo-token",
                        sov_bank::genesis::DEPLOYER.as_ref(),
                        sov_bank::genesis::SALT,
                    );
            let cmd = cmd_parser("Bank", &format!(
                r#"{{
                    "Transfer": {{
                        "to": "{}",
                        "coins": {{
                ...
                accounts[dest_idx].priv_key.pub_key().to_address::<sov_modules_api::Address>(),
                amount,
                token_address,
            )).unwrap();
            let tx = Transaction::<C>::new_signed_tx(
                &accounts[source_idx].priv_key,
                cmd,
                accounts[source_idx].nonce,
            );
            result.push(hex::encode(tx.try_to_vec().unwrap()));

            // Do the bookkeeping.
            accounts[source_idx].nonce += 1;
            accounts[dest_idx].amount += amount;
            accounts[source_idx].amount -= amount;
            if accounts[source_idx].amount == 0 {
                let _ = accounts.swap_remove(source_idx);
            }
        }
        result
    }
...
pub fn main() {
    ...
    match cli.command {
        ...
        Commands::Avalanche { number } => {
            let hex_encoded_txs = avalanche::gen(number);
            let blob = make_hex_blob(hex_encoded_txs.into_iter());
            println!("{}", blob)
        }
