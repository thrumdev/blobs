...
impl TreeBuilder {
	pub fn new() -> Self {
		Self { tree: NamespaceMerkleTree::new(), last_namespace: Namespace::with_namespace_id(0) }
	}
	...
	pub fn root(&mut self) -> TreeRoot {
		...
		TreeRoot { root: root.hash(), min_ns, max_ns }
	}

	pub fn proof(&mut self, namespace: Namespace) -> NamespaceProof {
		let (leaves, proof) = self.tree.get_namespace_with_proof(namespace.nmt_namespace_id());
