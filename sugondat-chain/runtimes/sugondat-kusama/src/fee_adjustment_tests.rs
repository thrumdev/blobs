...
fn multiplier_can_grow_from_zero() {
    ...
    let test = |mul_type: MultiplierType| {
        mul_type.run_with(
            Weight::from_parts(mul_type.target().ref_time() * 101 / 100, 0),
            || {
    ...
    let mul_fee = MultiplierType::Fee;
    // the block proof size is 1/100th bigger than target.
    mul_fee.run_with(
        Weight::from_parts(0, (mul_fee.target().proof_size() / 100) * 101),
        || {
            let next = mul_fee.runtime_multiplier_update(mul_fee.min_multiplier());
            assert!(
                next > mul_fee.min_multiplier(),
                "{:?} !> {:?}",
                next,
                mul_fee.min_multiplier()
            );
        },
    )
}
