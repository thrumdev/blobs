...
impl sov_rollup_interface::services::da::DaService for DaProvider {
	...
	async fn get_finalized_at(&self, height: u64) -> Result<Self::FilteredBlock, Self::Error> {
		...
		loop {
			...
			let header = client.backend().block_header(finalized_head.hash()).await?.unwrap();
		...
		let hash: subxt::utils::H256 =
			rpc_client.request("chain_getBlockHash", rpc_params![height]).await?;
		...
		Ok(types::Block { header, transactions, blob_proof })
	}
	...
	) -> (<Self::Spec as DaSpec>::InclusionMultiProof, <Self::Spec as DaSpec>::CompletenessProof) {
