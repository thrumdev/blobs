...
impl MockBuilder {
	...
	fn push_blob(&mut self, who: [u8; 32], namespace: Namespace, blob_hash: [u8; 32]) {
		...
		self.blobs.push(BlobMetadata {
			namespace,
			leaf: NmtLeaf { extrinsic_index: index as u32, who, blob_hash },
		});
	}
...
fn two_same_blobs() {
	...
	assert!(proof
		.verify(&[[2u8; 32], [2u8; 32]], tree.root(), Namespace::with_namespace_id(1))
		.is_ok());
}
...
fn empty() {
	...
	let proof = tree.proof(Namespace::with_namespace_id(1));
	assert!(proof.verify(&[], tree.root(), Namespace::with_namespace_id(1)).is_ok());
}
...
fn empty_absent_namespace_id() {
	let mut tree = MockBuilder::new().tree();
	let proof = tree.proof(Namespace::with_namespace_id(1));
	assert!(proof.verify(&[], tree.root(), Namespace::with_namespace_id(2)).is_ok());
}
...
fn wrong_namespace_id() {
	...
	let mut tree = b.tree();
	let proof = tree.proof(Namespace::with_namespace_id(2));
	assert!(proof.verify(&[], tree.root(), Namespace::with_namespace_id(1)).is_err());
}
